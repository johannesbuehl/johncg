...
// src/server/SequenceItems/SongFile.ts
...
var is_song_element = (x) => {
  if (typeof x !== "string") {
    return false;
  }
  const stem = x.split(" ", 1)[0];
  return verse_types.includes(stem.toLowerCase());
};
var SongFile = class {
  song_file_path;
  // private variables
  ...
  metadata = {
    ...
    Title: [],
    LangCount: 1
    // set it by default to 1. if there are more languages, they will be read from the header
   ...
   */
  parse_text_header(header) {
    const header_data = header.split(/\r?\n/);
  ...
  parse_song_text() {
    const raw_data_buffer = import_fs.default.readFileSync(this.song_file_path);
    ...
    const raw_data = import_iconv_lite.default.decode(raw_data_buffer, encoding);
    ...
    for (const data_block of data) {
      ...
      if (is_song_element(first_line_items[0].toLowerCase()) && first_line_items.length <= 2 && first_line_items.length > 0) {
        key = lines[0];
        lines.splice(0, 1);
        this.text_parts[key] = [];
      }
      ...
      const slide = Array.from(Array(Math.ceil(lines.length / this.metadata.LangCount)), () => []);
      lines.forEach((vv, ii) => {
        slide[Math.floor(ii / this.metadata.LangCount)].push(vv);
      });
      if (this.text_parts[key] !== void 0) {
  ...
  get part_title() {
    ...
    if (this.metadata.ChurchSongID !== void 0) {
  ...
  get_title_client() {
    const response = {
      type: "title",
      slides: 1
      // static, since there is always only one title-slide
    };
    return response;
  }
  get_part_client(part) {
    if (!is_song_element(part) || !this.avaliable_parts.includes(part)) {
      throw new ReferenceError(`'${part}' is no valid song-part`);
    }
    return {
...
// prototyping/main.ts
var search_dir = "D:/Coding_z1glr/liedtexte_git/";
function find_sng_files(pth) {
  const files = import_fs2.default.readdirSync(pth);
  const song_files2 = [];
  files.forEach((f) => {
    const ff = import_path.default.join(pth, f);
    const stat = import_fs2.default.statSync(ff);
    if (stat.isDirectory()) {
      song_files2.push(...find_sng_files(ff));
    } else if (import_path.default.extname(ff) === ".sng") {
      song_files2.push(new SongFile_default(ff));
    }
  });
  return song_files2;
}
var song_files = find_sng_files(search_dir);
var song_ids = {};
var song_text = {};
song_files.forEach((song) => {
  song_ids[song.song_file_path] = song.metadata.ChurchSongID;
  const this_song_text = [];
