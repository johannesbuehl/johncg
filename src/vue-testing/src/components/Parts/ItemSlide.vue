...
watch(() => [ props.active, props.scroll ], () => {
	scroll_into_view();
});
...
<template>
	<div ref="slide" class="slide_wrapper">
		<img class="media" :src="media" :style="{ 'aspect-ratio': aspect_ratio, 'opacity': media ? 'unset' : '0' }">
		<object v-if="template" class="template" :data="template?.template + '.html'" @load="$emit('template_load', $event.target as HTMLObjectElement)" />
